...
import type { ValueType, VueComponent } from '@/common/types/valueType';
  ...
  onMounted(() => {
    ...
    const watchSource =
      typeof currentValue.value === 'object' && currentValue.value !== null
        ? () => [
            (pickerRef?.value as VueComponent)?.isPopupVisible ?? popupVisibilityProp?.value,
            ...(currentValue.value as Array<ValueType | ValueType[]>).flat(),
          ]
        : () => [(pickerRef?.value as VueComponent)?.isPopupVisible ?? popupVisibilityProp?.value, currentValue.value];

    watch(watchSource, ([newIsPopupVisible, ...newCurrentValue], [prevIsPopupVisible, ...prevCurrentValue]) => {
      const newValue = newCurrentValue.reduce((prev, current) => prev + current, 0);
      const prevValue = prevCurrentValue.reduce((prev, current) => prev + current, 0);
      if (eventOnPopupClosing) {
        if (newValue !== prevValue) {
          isCurrentValueChanged = true;
          return;
        }
        if (!newIsPopupVisible && newIsPopupVisible !== prevIsPopupVisible && isCurrentValueChanged) {
